...
画面内容：以中国古典意境为核心，广角镜头缓慢展现远近层次分明的场景：远处群山如黛，近处水光潋滟、古树苍劲，天空云气流动，自然环境与诗句意象相呼应。中景镜头聚焦于符合{dynasty}时代服饰与气质的人物形象，人物的身份气质与诗中情感相匹配（如游子、隐士、将士、宫人等），服饰、发髻、佩饰均为古代样式。特写镜头捕捉关键意象（如月光、霜色、鸿雁、孤舟、残雪、落叶等），构图讲究留白与层次，整体画面稳定、连贯、符合物理常识。

时代与风格：整体风格为国风动画 / 水墨动画风格，重点使用中国古典水墨或工笔画语言，线条柔和，墨色层次丰富。禁止出现现代城市建筑、现代交通工具、现代霓虹灯牌、现代服饰或科幻元素。建筑采用{dynasty}时期特征的城楼、庭院、长廊、桥梁、亭台等，环境为山水、江河、古道、村落等传统场景，色调淡雅克制，以自然色和水墨晕染为主。

镜头节奏与运镜：镜头运动以缓慢推进、平稳横移、小幅环绕为主，偶尔使用俯拍或仰拍增强空间层次，但避免剧烈晃动和无意义的快速旋转。不同场景之间通过自然的景别变化或光线变化进行衔接，保持叙事连贯。整部视频中人物外观、服饰主色调和气质要保持前后一致，不能在镜头之间随机改变人物性别、年龄或脸部特征。

背景音乐：{music_desc}。

古诗朗诵：{narration_desc}。

转场效果：以淡入淡出、光影渐变、云雾掠过等柔和方式为主，禁止突然闪白、炫彩霓虹特效等强烈现代感效果。

环境音效：根据诗意加入恰当的环境音效，如鸟鸣、流水、风声、雨声、马蹄声、钟鼓声等，音量适中，不喧宾夺主，与画面和情感保持统一。

整体要求：整体风格古典统一、时代感准确，人物造型和场景在整部视频中保持一致，不出现违和的现代元素或逻辑冲突。画面精致、运镜稳定，音画同步，体现古典文学的美感，视频节奏与诗词韵律相匹配，情感表达{emotion}。"""
